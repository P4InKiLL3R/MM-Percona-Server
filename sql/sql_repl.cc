...
/**
  Execute a START SLAVE statement. Multi-Master By vagner@
  ...
  @retval 1 error
*/
int start_slave_all(THD* thd)
{
// I need mutex at this place???	
  Master_info *mi = 0;
  int i = 0;

  while ((mi=master_info_index->get_all_master_info_from_hash(i)) != 0 )
  {
    sql_print_information("[Multi-Master] Start Slave IDX:'%d'... Search in HASH", i);	  
...
/**
  Execute a START SLAVE 'idx' statement.
...
/**
  Execute a STOP SLAVE 'idx' statement.
...
// I need mutex at this place???	
  ...
  while ((mi=master_info_index->get_all_master_info_from_hash(i)) != 0 )
  {
    ...
    sql_print_information("[Multi-Master] Stop Slave IDX:'%d'... Search in HASH", i);	  
    i++;
  }
}
...
/* Multi-Master By P.Linux */
  ...
  if (mi->sign != '\0') // if use CHANGE MASTER 'xxx' TO
  {
...
/* Multi-Master By P.Linux */
  fn_format(fname, buf_relay_log_info_file, mysql_data_home, "", 4+32);
  if (my_stat(fname, &stat_area, MYF(0)) && my_delete(fname, MYF(MY_WME)))
  {
    error=1;
    goto err;
...
/* Multi-Master By P.Linux */
  ...
  sql_print_information("Sign:%s, Master_info:%s, Relay_info:%s", 
