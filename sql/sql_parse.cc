...
/* Multi-Master By P.Linux */
    LEX_MASTER_INFO* lex_mi= &thd->lex->mi;
/* Add show all slaves for Multi-Master By vagner@ */
    if (lex_mi->sign == NULL)
	res = show_slave_all(thd);
    else
    {
      Master_info *mi = master_info_index->get_master_info_from_hash(lex_mi->sign);
      if (mi != NULL)
      {
...
/* Multi-Master By P.Linux */
    ...
    if (lex_mi->sign != NULL)
    {	    
      mi = master_info_index->get_master_info_from_hash(lex_mi->sign);
      if (mi != NULL) 
...
/* Multi-Master By P.Linux */
   ...
   else
   {
	...
	if (result != 1 && mi->sign)
	    master_info_index->remove_master_info_from_hash(mi->sign);
