...
bool show_slave_all(THD* thd)
{
  ...
  field_list.push_back(new Item_empty_string("Slave_IO_State",
                                                     14));
  field_list.push_back(new Item_empty_string("Slave_is_Running",
                                                      3));
  field_list.push_back(new Item_empty_string("Master_Sign",
                                                    100));
  ...
  ulong i = 0;
//  ulong ii = 0;
  pthread_mutex_lock(&LOCK_slave_status);
  Master_info *mi = 0;

  while ((mi=master_info_index->get_all_master_info_from_hash(i)) != 0 ) 
  {
#if defined DEBUG	  
    sql_print_information("[Multi-Master] Reading IDX:'%lu'... Search in HASH", i);
    //Master_info *mi= master_info_index->get_all_master_info_from_hash(i);
    //	  if (mi != NULL) {
    sql_print_information("[Multi-Master] MI not NULL ");
#endif    
    if (mi->host[0]) 
    {
      DBUG_PRINT("info",("host is set: '%s'", mi->host));
		    ...
		    protocol->store(mi->sign, &my_charset_bin);
...
#if defined DEBUG	  
			sql_print_information("[Multi-Master] Reading IDX:'%lu'... Search in HASH Done", i);
#endif    
			  if (my_net_write(&thd->net, (uchar*) thd->packet.ptr(), packet->length()))
				  DBUG_RETURN(TRUE);
		  }
#if defined DEBUG	  
		    sql_print_information("[Multi-Master] MI not NULL End");
#endif    
		    ++i;
#if 0
	  }
	  ...
	  {
		    sql_print_information("[Multi-Master] MI NULL ");
		  ii=1;
		    sql_print_information("[Multi-Master] MI NULL END");
		    break;
		    DBUG_RETURN(TRUE);
	  }
#endif
...
#if defined DEBUG	  
    sql_print_information("[Multi-Master] Cycle is DONE");
#endif    
  pthread_mutex_unlock(&LOCK_slave_status);
  my_eof(thd);
  DBUG_RETURN(FALSE);
}
